...
import "github.com/sevagh/k-ary-tree"

func (q *QuadTree) TreeToSlice() []uint32 {
	treeArrRep := make([]uint32, q.Leaves()+2)

	treeArrRep[0] = uint32(q.Root.Key().(uint32)) //root = index 0
...
func DFS(q *karytree.Node, index int, storage *[]uint32) {
	if q.NthChild(NE) != nil { //not a leaf node
		(*storage)[4*index+1] = uint32(q.NthChild(NE).Key().(uint32))
		DFS(q.NthChild(NE), 4*index+1, storage)

		(*storage)[4*index+2] = uint32(q.NthChild(NW).Key().(uint32))
		DFS(q.NthChild(NW), 4*index+2, storage)

		(*storage)[4*index+3] = uint32(q.NthChild(SE).Key().(uint32))
		DFS(q.NthChild(SE), 4*index+3, storage)

		(*storage)[4*index+4] = uint32(q.NthChild(SW).Key().(uint32))
